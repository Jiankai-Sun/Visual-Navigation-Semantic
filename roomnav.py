...
from House3D import House, Environment, MultiHouseEnv, objrender
...
time_penalty_reward = 0.2  # 0.1   # penalty for each time step
...
ALLOWED_PREDICTION_ROOM_TYPES = dict(outdoor=0, indoor=1, kitchen=2, dining_room=3,
                                     living_room=4, bathroom=5, bedroom=6, office=7,
                                     storage=8)
#################
# Util Functions
#################
...
class RoomNavTask(gym.Env):
    def __init__(self, env,
        ...
        self.room_table=np.eye(len(ALLOWED_PREDICTION_ROOM_TYPES))
        self.target = self.room_table[ALLOWED_PREDICTION_ROOM_TYPES[self.house.targetRoomTp]].reshape([1,len(ALLOWED_PREDICTION_ROOM_TYPES)])
    ...
    def reset(self, target=None):
        ...
        self.target = self.room_table[ALLOWED_PREDICTION_ROOM_TYPES[self.house.targetRoomTp]]
        self.inside_target_room = 0
    ...
    def get_semantic(self):
        semantic = self.env.render(mode='semantic')
        return semantic

    def get_rgb(self):
        rgb = self.env.render(mode='rgb')
        return rgb
